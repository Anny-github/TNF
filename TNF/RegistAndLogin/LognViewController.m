...
//        logn.layer.masksToBounds = YES;
    ...
    //微信
        UIButton *wxlogn = [UIButton buttonWithType:UIButtonTypeCustom];
        wxlogn.frame = CGRectMake(0, kScreenHeight - 45 *ratioHeight , kScreenWidth, 45 *ratioHeight);
        //        logn.layer.cornerRadius = 20 * ratioHeight;
        //        logn.layer.masksToBounds = YES;
        wxlogn.backgroundColor = UIColor1(<#蓝#>);
        wxlogn.tag = 1000;

        [wxlogn addTarget:self action:@selector(LognAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:wxlogn];
        
        UIImageView  *imageView3 = [[UIImageView alloc]initWithFrame:CGRectMake((wxlogn.width - 30 * ratioHeight -  75 * ratioWidth) / 2.0  ,(wxlogn.height - 30 * ratioHeight) / 2.0  ,30 * ratioHeight , 30 * ratioHeight)];
        imageView3.contentMode = UIViewContentModeScaleAspectFit;
        imageView3.image = [UIImage imageNamed:@"weixin"];
        [wxlogn addSubview:imageView3];
        
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(imageView3.right + 5 * ratioWidth, imageView3.top, 70 * ratioWidth, imageView3.height)];
        label.font = [UIFont systemFontOfSize:17 * ratioWidth];
        label.text = @"微信登录";
        label.textColor = [UIColor whiteColor];
        [wxlogn addSubview:label];
...
- (void)initGuide
{
    ...
    [_scrollView setContentSize:CGSizeMake(kScreenWidth*array.count, kScreenHeight)];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    int count = (scrollView.contentOffset.x + kScreenWidth/2.0)/kScreenWidth;
...
- (void)LognAction:(UIButton *)button{
    ...
    if (button.tag == 1000) {
        //有微信
        [[TNFLogin shared] loginWithType:TNFLoginTypeWEIXIN];
...
- (void)_zhanghaoLogin
{
    ...
    if (_textFieldName.text.length == 0) {
        [TNFPublicTool HUDWithString:@"请填写账号"];

    }
    if (_textFielePassd.text.length == 0) {
        [TNFPublicTool HUDWithString:@"请填写密码"];
    }
