...
@interface OralController ()<BgViewdelegate,BgView1delegate>
...
-(void)isSignIn{
    ...
    [WXDataService requestAFWithURL:Url_getIsSignin params:@{@"member_id":[UserDefaults valueForKey:Userid]} httpMethod:@"POST" finishBlock:^(id result) {
        
        NSDictionary *dic = result[@"result"];
        if ([dic[@"isSignIn"] integerValue] == 0) {
            NSString *su= [UserDefaults objectForKey:subject];
            if([su intValue] != 0) {
                NSString *amountSignInTo= [NSString stringWithFormat:@"每日练题好礼相送 \n 明日奖励预告:+%@个福币",dic[@"amountSignInTo"]];
                BgView1 *bg1 = [[BgView1 alloc]initWithFrame:self.view.frame TodayFubi:[dic objectForKey:@"amountSignIn"] TomorrowFubi:amountSignInTo];
                bg1.BgViewdelegate =self;
                [kWindow addSubview:bg1];
...
- (void)_initViews
{
    ...
    TFPicScrollView *picView = [TFPicScrollView picScrollViewWithFrame:CGRectMake(0, 0, kScreenWidth, 160 * ratioHeight) WithImageUrls:marray];
...
//        imageView.backgroundColor = [UIColor grayColor];
        imageView.tag = 100 + i;
        imageView.userInteractionEnabled = YES;
        [imageView sd_setImageWithURL:[NSURL URLWithString:list.thumb] placeholderImage:nil];
