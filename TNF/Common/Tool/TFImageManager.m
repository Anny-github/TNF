...
@implementation TFImageManager

+ (instancetype)shareManager {
    
    static TFImageManager *instance;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[TFImageManager alloc] init];
    });
...
- (void)downloadImageWithUrlString:(NSString *)urlSting {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue >= 7.0) {
        ...
        [[[NSURLSession sharedSession] dataTaskWithURL:[NSURL URLWithString:urlSting] completionHandler:^(NSData *  data, NSURLResponse *  response, NSError *  error) {
        ...
        [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlSting]] queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *  response, NSData *  data, NSError *  connectionError) {
