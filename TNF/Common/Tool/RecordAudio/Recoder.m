...
- (void)startRecoder
{
    ...
    //创建录音对象
    self.audioRecoder = [[AVAudioRecorder alloc] initWithURL:audioRecordingURL settings:[self audioRecordingSettings] error:&error];
    ...
    if ([_audioRecoder record])
    {
        NSLog(@"start record");
        // 开始录音的时候计数器初始化
        _times = 0;
        _counterTime = 0;
        self.recoderTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(timerCountingRecord) userInfo:nil repeats:YES];
    }
    else
    {
...
//        NSData *fileData = [NSData dataWithContentsOfFile:[self filePathOfRecordAudioView] options:NSDataReadingMappedIfSafe error:&readingError];
        ...
        //初始化播放器
        _audioPlayer = [[AVAudioPlayer alloc] initWithData:fileData error:&playbackErroor];
