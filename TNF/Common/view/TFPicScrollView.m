...
#import "TFImageManager.h"

@interface TFPicScrollView ()
@property (nonatomic,copy) NSArray *imageData;

@end

@implementation TFPicScrollView
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[TFPicScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (void)preparePageControl {
    ...
    UIPageControl *page = [[UIPageControl alloc] initWithFrame:CGRectMake(0,(self.height-10),myWidth,7)];
...
- (void)changeImageLeft:(NSInteger)LeftIndex center:(NSInteger)centerIndex right:(NSInteger)rightIndex {
    ...
    if (_isNetwork) {
        [_leftImageView sd_setImageWithURL:[NSURL URLWithString:_imageData[LeftIndex]] placeholderImage:nil];
        [_centerImageView sd_setImageWithURL:[NSURL URLWithString:_imageData[centerIndex]] placeholderImage:nil];
        [_rightImageView sd_setImageWithURL:[NSURL URLWithString:_imageData[rightIndex]] placeholderImage:nil];
...
- (void)scorll {
    CGFloat offsetX =  _scrollView.contentOffset.x/myWidth * myWidth + myWidth;
    //_scrollView.contentOffset.x + myWidth
...
- (void)getImage { //初始化的时候就下载图片缓存，避免到时候再下载出现卡顿
    
    UIImageView *imgV = [[UIImageView alloc]init];
    for (NSString *urlSting in _imageData) {
        [imgV sd_setImageWithURL:[NSURL URLWithString:urlSting] placeholderImage:nil];
        [[TFImageManager shareManager] downloadImageWithUrlString:urlSting];
    }
