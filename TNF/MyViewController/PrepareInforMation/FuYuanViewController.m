...
//            self.datelist = @[@"",_model.text,_model.isGet1,_model.isGet2,_model.buyList];
            ...
            if ([_model.isget1 integerValue] == 0) {
                [cellBtn1 setTitle:@"领取" forState:UIControlStateNormal];
                cellBtn1.userInteractionEnabled = YES;
                [cellBtn1 setBackgroundColor:UIColor3(金色)];
            } else {
                [cellBtn1 setTitle:@"已领取" forState:UIControlStateNormal];
                cellBtn1.userInteractionEnabled = NO;
                [cellBtn1 setBackgroundColor:UIColor4(所有的灰色)];
            }
            
            if ([_model.isget2 integerValue] == 0) {
                [cellBtn2 setTitle:@"分享" forState:UIControlStateNormal];
                cellBtn2.userInteractionEnabled = YES;
                [cellBtn2 setBackgroundColor:UIColor3(金色)];
            } else {
                [cellBtn2 setTitle:@"已分享" forState:UIControlStateNormal];
                cellBtn2.userInteractionEnabled = NO;
                [cellBtn2 setBackgroundColor:UIColor4(所有的灰色)];
            }
...
//            imageView1.hidden = YES;
            ...
            //领取按钮
            cellBtn1 = [UIButton buttonWithType:UIButtonTypeCustom];
            cellBtn1.frame = CGRectMake(kScreenWidth-70*ratioHeight, _label1.bottom, 60*ratioHeight, 26*ratioHeight);
            cellBtn1.titleLabel.font = [UIFont systemFontOfSize:13 * ratioWidth];
//            [cellBtn1 setTitle:@"领取" forState:UIControlStateNormal];
//            [cellBtn1 setBackgroundColor:UIColor3(金色)];
            cellBtn1.tag = 10;
            [cellBtn1 addTarget:self action:@selector(cellBtnClick:) forControlEvents:UIControlEventTouchUpInside];
            cellBtn1.layer.cornerRadius = 13 * ratioHeight;
            
            // 按钮边框宽度
            cellBtn1.layer.borderWidth = 0;
            [cell addSubview:cellBtn1];
            ...
            //领取按钮
            cellBtn2 = [UIButton buttonWithType:UIButtonTypeCustom];
            cellBtn2.frame = CGRectMake(kScreenWidth-70*ratioHeight, _label1.bottom, 60*ratioHeight, 26*ratioHeight);
            cellBtn2.titleLabel.font = [UIFont systemFontOfSize:13 * ratioWidth];
//            [cellBtn2 setTitle:@"领取" forState:UIControlStateNormal];
//            [cellBtn2 setBackgroundColor:UIColor3(金色)];
            cellBtn2.tag = 11;
            [cellBtn2 addTarget:self action:@selector(cellBtnClick:) forControlEvents:UIControlEventTouchUpInside];
            cellBtn2.layer.cornerRadius = 13 * ratioHeight;
            
            // 按钮边框宽度
            cellBtn2.layer.borderWidth = 0;
            [cell addSubview:cellBtn2];
            ...
            if ([result[@"status"] integerValue] == 0) {
                ...
                [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:image] options:SDWebImageDownloaderLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                    ...
                    if (finished) {
                        ...
                        [UMSocialData defaultData].extConfig.sinaData.shareText = [NSString stringWithFormat:@"%@%@",connent,url];
//                没用   [UMSocialData defaultData].extConfig.sinaData.urlResource.url = url;
                        ...
                        [TNFPublicTool HUDWithString:@"亲，网速不好哦！！！"];
...
-(void)didFinishGetUMSocialDataInViewController:(UMSocialResponseEntity *)response
{
    ...
    if(response.responseCode == UMSResponseCodeSuccess)
    {
        ...
        [WXDataService requestAFWithURL:Url_setAmountShare params:parmas httpMethod:@"POST" finishBlock:^(id result) {
            ...
            if ([result[@"status"] integerValue] == 0) {
                [TNFPublicTool HUDWithString:@"分享成功"];
        ...
        _isFenxiang = YES;
        [TNFPublicTool HUDWithString:@"分享失败"];        
    }
