...
//    [imageView sd_setImageWithURL:[NSURL URLWithString:self.dic[@"headimgurl"]]];
    ...
    button.frame = CGRectMake(kScreenWidth - 60 - 20,40, 60, 30);
...
-(void)setVoiceDetail:(VoiceDetailModle *)voiceDetail{
    _voiceDetail = voiceDetail;
    self.mp3_list = _voiceDetail.mp3_list;
    self.Url = _voiceDetail.info.teacher_headimgurl;
    self.title = _voiceDetail.info.teacher_nickname;
    self.teacher_member_id = _voiceDetail.info.teacher_member_id;
    self.ID = _voiceDetail.subject_info.ID;
    self.selectionStyle = UITableViewCellSelectionStyleNone;
    self.isDa = !_voiceDetail.info.is_self;
    
    self.ISDASHANG = [_voiceDetail.info.is_dashang boolValue];
