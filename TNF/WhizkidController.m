...
//    [bgimageView addSubview:blurView];
    ...
    playBtn.frame = CGRectMake(0, 0, 60 * ratioHeight, 60 * ratioHeight);
    playBtn.layer.masksToBounds = YES;
    playBtn.layer.cornerRadius = 60 / 2.0 * ratioHeight;
    [playBtn setImage:[UIImage imageNamed:@"play_icon"] forState:UIControlStateNormal];
    [playBtn setImage:[UIImage imageNamed:@"play_icon"] forState:UIControlStateSelected];
    [playBtn setImage:[UIImage imageNamed:@"play_icon"] forState:UIControlStateHighlighted];

    [playBtn addTarget:self action:@selector(mp3button:) forControlEvents:UIControlEventTouchUpInside];
    [bgimageView addSubview:playBtn];
    playBtn.center = bgimageView.center;
    ...
    timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, playBtn.bottom + 10, kScreenWidth, 13 * ratioHeight)];
...
//            }
            self.com_list = dic[@"com_list"];  //老师点评
            NSArray *coms = dic[@"comment_list"]; //评论列表
            ...
            NSMutableArray *array1 = [NSMutableArray array]; //评论 的frame
