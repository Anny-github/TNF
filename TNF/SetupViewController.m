...
-(UILabel *)cacheLabel{
    if (_cacheLabel == nil) {
        _cacheLabel = [[UILabel alloc]initWithFrame:CGRectMake(kScreenWidth-100*ratioHeight, (45*ratioHeight - 20 * ratioHeight) / 2.0, 70*ratioHeight, 20*ratioHeight)];
        _cacheLabel.font = [UIFont systemFontOfSize:13*ratioHeight];
        _cacheLabel.textAlignment = NSTextAlignmentRight;
        _cacheLabel.textColor = UIColor6(正文小字);
        _cacheLabel.text = [NSString stringWithFormat:@"%.2fM",[[SDImageCache sharedImageCache]getSize]/1024.0/1024.0];
    }
    return _cacheLabel;
}
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if ([[[UIApplication sharedApplication]scheduledLocalNotifications] count] !=0 ) {
        _nofiLabel.hidden = YES;
    }else{
        _nofiLabel.hidden = NO;
    }
...
- (void)viewDidLoad {
    ...
    //图片数组
    _images = @[@"setlb_01.png",@"setlb_02.png",@"setlb_03.png",@"setlb_04.png",@"report"];
    
    //文字数组
    _titles1 = @[@"我的福币",@"备考信息",@"练习记录",@"我的课程",@"数据报告"];
    _titles = @[@"每日答题提醒",@"清理缓存",@"加入我们",@"意见反馈",@"协议与条款"];
       //导航栏标题
    self.text = @"设置";
...
//            [MBProgressHUD showSuccess:result[@"msg"] toView:[UIApplication sharedApplication].keyWindow];
            ...
            _touxiangImageView.layer.cornerRadius = _touxiangImageView.width/2.0;
...
-(void)_initViews
{
    ...
    _setupTableView.tableHeaderView = ({
        ...
        _nofiLabel = [[UILabel alloc]initWithFrame:CGRectMake(kScreenWidth-100*ratioHeight, (45*ratioHeight - 20 * ratioHeight) / 2.0, 70*ratioHeight, 20*ratioHeight)];
        _nofiLabel.text = @"未开启";
        _nofiLabel.font = [UIFont systemFontOfSize:13*ratioHeight];
        _nofiLabel.textAlignment = NSTextAlignmentRight;
        _nofiLabel.textColor = UIColor6(<#正文小字#>);
...
//            lineview.hidden = YES;
            ...
            if (indexPath.row == 0) {
                if ([[[UIApplication sharedApplication]scheduledLocalNotifications] count] == 0) {
                    ...
                    [cell.contentView addSubview:_nofiLabel];
                }
            }else if (indexPath.row == 1) {
...
//    [tableView deselectRowAtIndexPath:indexPath animated:YES];// 取消选中
    ...
    {
        if(indexPath.row == 0){ //答题题型
            DayTimeAlertController *timeVC = [[DayTimeAlertController alloc]init];
            [self.navigationController pushViewController:timeVC animated:YES];
            
        }else if(indexPath.row == 1){
            [[[SDWebImageManager sharedManager]imageCache] clearDiskOnCompletion:^{
                [[[SDWebImageManager sharedManager]imageCache] clearMemory];
                [TNFPublicTool HUDWithString:@"清除成功"];
...
//                [MBProgressHUD showSuccess:@"清除成功" toView:kWindow];
//                self.cacheLabel.text = @"";
//            }];
//            
