...
- (void)_loadData
{
    ...
    [WXDataService requestAFWithURL:Url_getSubjectInfo params:params httpMethod:@"POST" finishBlock:^(id result) {
        if(result){
            ...
            if (ma.count !=10) { //表示只有一页数据，不显示footer
    ...
    [WXDataService requestAFWithURL:Url_getAmountUploadMp3Info params:params httpMethod:@"POST" finishBlock:^(id result) {
        if(result){
            ...
            if (amount < 5) {
                ...
                //collectionView 的cell下拉，在cell内部开始录音
                DetailSubjectCell *cell = [[_collectionView visibleCells] firstObject];
                NSLog(@"cellindexpath--------%d",[[_collectionView indexPathForCell:cell] row]);
                cell.fromCellSelcted = NO;
...
#pragma mark ---------LXViewDelegate，录音发布成功-------
- (void)clickOK
{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (type == 0) {
        ...
        VoiceModel *model1 = array[indexPath.row];
        whVC.ID = model1.ID;
        
//        VoiceDetailController *vc = [[VoiceDetailController alloc]init];
//        vc.model = whVC.model;
//        vc.ID = model1.ID;
...
- (void)clickindex:(int)index
{
    ...
    if (index == 0) {
        ...
        [self loadContent]; //解题思路，显示在label上，作为tableView的tableFooterView
...
- (void)loadContent
{
    _tableView.footer = nil;
    HeaderModel *hearder = self.collectionDatalist[self.selectedIndexPath.row];
    if (hearder.contents != nil) {  // 已经请求过该数据，且添加到model.contents属相
        [self addfooter];  //在相关素材赋值显示时，必须删除tableViewfooterView,所以要重新添加
