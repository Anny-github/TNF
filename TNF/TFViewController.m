...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(examInfoChanged) name:Noti_ExamInfoChange object:nil];
...
//    [self CreateBjView];
    ...
    //监听是否有老师点评我的录音
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(myTestAnswerd:) name:Noti_MyTestAnswerd object:nil];
}

#pragma mark --通知
//有老师评价我的题
-(void)myTestAnswerd:(NSNotification*)nofi{
    
    NSDictionary *userInfo = nofi.object;
    if ([userInfo[@"myTest"] integerValue] == 1) {
        _yuandian.hidden = NO;
...
- (void)createTablewView{
    ...
    footV.backgroundColor = [MyColor colorWithHexString:@"0x000000"];
...
//        NSArray *color = @[@"0x0172fe",@"0xffad01",@"0x0172fe",@"0xff1e6f"];
    ...
    if (Yindex==0) {
        ...
        if (nameAry.count%3 == 0) {
            Y = ((x+65*ratioHeight/2)+ratioHeight*50 + height*(Yindex) + 25*ratioHeight/2 *(Yindex - 1))+10;
        } else {
           Y = ((x+65*ratioHeight/2)+ratioHeight*50 + height*(Yindex + 1) + 25*ratioHeight/2 *(Yindex - 1))+10;
    ...
    //为你推荐
    UILabel *recommendLabel = [WXLabel UIlabelFrame:CGRectMake(0, 0, ratioWidth*50*2, ratioHeight*40/2) textColor:[UIColor whiteColor] textFont:[UIFont systemFontOfSize:13*ratioHeight] labelTag:600];
    ...
    //更多
    UIButton *MoreLabel = [[UIButton alloc]initWithFrame:CGRectMake(more.width-ratioWidth*50*2, 0, ratioWidth*50*2, ratioHeight*40/2)];
    ...
    if (recommendListAry.count !=0) {
        ...
        self.Collectionbottom = more.bottom;
    }
